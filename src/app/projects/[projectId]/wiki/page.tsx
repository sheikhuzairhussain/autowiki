...
export default function WikiRedirectPage({ params }: WikiRedirectPageProps) {
  ...
  useEffect(() => {
    ...
    if (wiki && wiki.sections.length > 0 && wiki.sections[0].pages.length > 0) {
      const firstSection = wiki.sections[0];
      const firstPage = firstSection.pages[0];
      router.replace(
        `/projects/${projectId}/wiki/${firstSection.slug}/${firstPage.slug}`
      );
    }
